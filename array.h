...
#define array_init_capacity(array, _capacity)                                                 \
	do {                                                                                        \
		array_header_t *header = malloc(sizeof(*(array)) * (_capacity) + sizeof(array_header_t)); \
		if (header == NULL) {                                                                     \
...
#define array_append(array, item)                                                        \
	do {                                                                                   \
		...
		if (header->count >= header->capacity) {                                             \
			header->capacity *= 2;                                                             \
			array_header_t *tmp =                                                              \
					realloc(header, sizeof(*(array)) * header->capacity + sizeof(array_header_t)); \
			if (tmp == NULL) {                                                                 \
...
#define array_length_set(array, length) ((array_header_t *) (array) - 1)->count = length
